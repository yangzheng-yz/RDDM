...
class UnetRes(nn.Module):
    def __init__(
        ...
        if self.share_encoder == 1:
            ...
            self.downs = nn.ModuleList([])
            self.ups = nn.ModuleList([])
            self.ups_no_skip = nn.ModulceList([])
            num_resolutions = len(in_out)
            ...
            self.unet1=Unet(dim,
                              ...
                              condition=condition,
                              input_condition=input_condition)
            # self.clipmodel = CLIPDensePredT(version='ViT-B/16', reduce_dim=64, complex_trans_conv=True).to(torch.device('cuda'))
    ...
    def forward(self, x, time,prompt='a photo of shadow region',x_self_cond=None):
        if self.share_encoder == 1:
            if self.self_condition:
                x_self_cond = default(x_self_cond, lambda: torch.zeros_like(x))
                x = torch.cat((x_self_cond, x), dim=1)
            ...
            # prompt='a photo of a shadow region'
            # t=torch.tensor([1]).to(torch.device('cuda'))
            # x_noise=x[:,:3,:,:]
            # x_f=self.clipmodel(x[:,3:,:,:],prompt)
            # x_f=torch.cat([x_noise,x_f],dim=1)
...
class ResidualDiffusion(nn.Module):
    ...
    def predict_start_from_xinput_noise(self, x_t, t, x_input, noise):
        return (
            (x_t-extract(self.alphas_cumsum, t, x_t.shape)*x_input -
             extract(self.betas_cumsum, t, x_t.shape) * noise)/(1-extract(self.alphas_cumsum, t, x_t.shape))
        )
    ...
    def model_predictions(self, x_input, x, t, x_input_condition=0, x_self_cond=None, clip_denoised=True):
        ...
        elif self.objective == "pred_xinput_noise":
            pred_noise = model_output[0]
            pred_res = x_input - \
                self.predict_start_from_xinput_noise(x, t, x_input, pred_noise)
    ...
    @torch.no_grad()
    def p_sample_loop(self, x_input, shape):
        if self.input_condition:
        ...
        if self.condition:
            return unnormalize_to_zero_to_one([input_add_noise, img])
        else:
            return unnormalize_to_zero_to_one([img])

    @torch.no_grad()
    def ddim_sample(self, x_input, shape):
        if self.input_condition:
            x_input_condition = x_input[1]
        else:
            x_input_condition = 0
    ...
    def p_losses(self, imgs, t, noise=None):
        ...
        if self.self_condition and random.random() < 0.5:
            with torch.no_grad():
                x_self_cond = self.model_predictions(
                    x_input, x, t, x_input_condition).pred_x_start
                x_self_cond.detach_()
        ...
        if u_loss:
            ...
            for i in range(len(model_out)):
                L=self.loss_fn(model_out[i], target[i], reduction='none')
                if i==0:
                    noise_loss=noise_loss+L
                else:
                    res_loss=res_loss+L
                loss = loss + L
        loss = reduce(loss, 'b ... -> b (...)', 'mean')
        noise_loss=reduce(noise_loss,'b ... -> b (...)', 'mean')
        res_loss=reduce(res_loss,'b ... -> b (...)', 'mean')
        return loss.mean(),noise_loss.mean(),res_loss.mean()
        ...
        *,
        ...
        if self.condition:
            if len(folder) == 2:
                ...
                # test_input
                generation=False
                artifact_source_path="/home/Data/Medical-Public/NIRII_denoising/NIRI_to_NIRII/For_argumentation/et_2-500ms-250_550-1000ms-10_1500-5000ms-8_1800LP_contrast.raw"
                ...
                ds = dataset([folder[1]], self.image_size,augment_flip=True, convert_image_to=convert_image_to, condition=11, equalizeHist=equalizeHist, crop_patch=crop_patch, sample=True, generation=generation, \
            ...
            if len(folder) == 3:
                ...
                ds = Dataset(folder[-1], self.image_size,
                             augment_flip=False, convert_image_to=convert_image_to, condition=0, equalizeHist=equalizeHist,crop_patch=crop_patch,sample=False)
                ...
                ds = Dataset(trian_folder, self.image_size, augment_flip=augment_flip,
                             convert_image_to=convert_image_to, condition=1,equalizeHist=equalizeHist,crop_patch=crop_patch)
                self.dl = cycle(self.accelerator.prepare(DataLoader(ds, batch_size=train_batch_size,
                                shuffle=True, pin_memory=True, num_workers=4)))
            elif len(folder) == 4:
                ...
                ds = Dataset(folder[2:4], self.image_size,
                             augment_flip=False, convert_image_to=convert_image_to, condition=1, equalizeHist=equalizeHist,crop_patch=crop_patch,sample=True)
                trian_folder = folder[0:2]
                ...
                ds = Dataset(folder[3:6], self.image_size,
                             augment_flip=False, convert_image_to=convert_image_to, condition=2, equalizeHist=equalizeHist,crop_patch=crop_patch,sample=False)
                ...
                ds = Dataset(trian_folder, self.image_size, augment_flip=augment_flip,
                             convert_image_to=convert_image_to, condition=2, equalizeHist=equalizeHist,crop_patch=crop_patch)
                self.dl = cycle(self.accelerator.prepare(DataLoader(ds, batch_size=train_batch_size,
                                shuffle=True, pin_memory=True, num_workers=4)))
        else:
            ...
            ds = Dataset(trian_folder, self.image_size, augment_flip=augment_flip,
                         convert_image_to=convert_image_to, condition=0, equalizeHist=equalizeHist,crop_patch=crop_patch)
            self.dl = cycle(self.accelerator.prepare(DataLoader(ds, batch_size=train_batch_size,
                            shuffle=True, pin_memory=True, num_workers=4)))
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        loss,noise_loss,res_loss = self.model(data)
                        loss = loss / self.gradient_accumulate_every
                        noise_loss=noise_loss/self.gradient_accumulate_every
                        res_loss=res_loss/self.gradient_accumulate_every
                        total_loss = total_loss + loss.item()
                        total_noiseloss=total_noiseloss+noise_loss.item()
                        total_resloss=total_resloss+res_loss.item()
                ...
                pbar.set_description(f'noise_loss: {total_noiseloss:.4f},res_loss: {total_resloss:.4f}')
                # pbar.set_description('noise_loss:{},res_loss:{}'.format(total_noiseloss,total_resloss,'.4f'))               
                pbar.update(1)
    ...
    def sample(self, milestone):
        ...
        with torch.no_grad():
            ...
            if self.condition_type == 0:
                ...
                show_x_input_sample = x_input_sample
                x_input_sample = x_input_sample[1:]#len(folder)==4
            ...
            all_images_list = show_x_input_sample+list(
                map(lambda n: self.ema.ema_model.sample(x_input_sample, batch_size=n), batches))[0]

            all_images = torch.cat(all_images_list, dim=0)

            file_name = f'sample-{milestone}.png'
            utils.save_image(all_images, str(
                self.results_folder / file_name), nrow=int(math.sqrt(self.num_samples)))
