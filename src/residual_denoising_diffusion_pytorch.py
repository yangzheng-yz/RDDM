...
class ResidualDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, x_input, shape, last=True, test_sampling_timesteps=5):
        if self.input_condition:
    ...
    @torch.no_grad()
    def ddim_sample(self, x_input, shape, test_sampling_timesteps=5, last=True):
        if self.input_condition:
            x_input_condition = x_input[1]
        else:
            x_input_condition = 0
        ...
        sampling_timesteps = test_sampling_timesteps
        ...
        x_start = None
        type = "use_pred_noise" # use_pred_noise

        if not last: # save intermedium results
            img_list = []
        ...
        for _time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            time_cond = torch.full(
                (batch,), _time, device=device, dtype=torch.long)
            self_cond = x_start if self.self_condition else None
            start_time = clock.time()
            preds = self.model_predictions(
                x_input, img, time_cond, x_input_condition, self_cond)
            end_time = clock.time()
            # print(f"The processing time for one sample step: {(end_time - start_time) * 1000}ms")
            pred_res = preds.pred_res
            pred_noise = preds.pred_noise
            x_start = preds.pred_x_start
            ...
            alpha_cumsum = self.alphas_cumsum[_time]
            alpha_cumsum_next = self.alphas_cumsum[time_next]
            alpha = alpha_cumsum-alpha_cumsum_next

            betas2_cumsum = self.betas2_cumsum[_time]
            betas2_cumsum_next = self.betas2_cumsum[time_next]
            betas2 = betas2_cumsum-betas2_cumsum_next
            betas = betas2.sqrt()
            betas_cumsum = self.betas_cumsum[_time]
            betas_cumsum_next = self.betas_cumsum[time_next]
            sigma2 = eta * (betas2*betas2_cumsum_next/betas2_cumsum)
            sqrt_betas2_cumsum_next_minus_sigma2_divided_betas_cumsum = (
                betas2_cumsum_next-sigma2).sqrt()/betas_cumsum
        ...
        # print(f"img_list length in ddim_sample is {len(img_list)}")
        ...
        if self.condition:
            if not last:
                ...
                img_list = [input_add_noise, img]
            # print(f"Debug img_list: {len(img_list)}")
            return unnormalize_to_zero_to_one(img_list)
        else:
            if not last:
                img_list = img_list
    ...
    @torch.no_grad()
    def sample(self, x_input=0, batch_size=16, last=True, test_sampling_timesteps=5):
        ...
        if self.condition:
            ...
            # print(f"Debug x_input channels: {channels}")
        else:
            size = (batch_size, channels, image_size, image_size)
        return sample_fn(x_input, size, last=last, test_sampling_timesteps=test_sampling_timesteps)
    ...
    def p_losses(self, imgs, t, noise=None):
        ...
        if not self.condition:
            ...
            if self.input_condition:
                ...
                x_in = torch.cat((x, x_input), dim=1) # here
        ...
        if self.objective == 'pred_res_noise':
            ...
            # print(f"Debug pred_res: {type(pred_res)}")
            # print(f"Debug pred_noise: {type(pred_noise)}")
        elif self.objective == 'pred_x0_noise':
        ...
        if u_loss:
            ...
            for i in range(len(model_out)):
                # print(f"Debug model_out type is: {type(model_out[i])}") #, size is: {model_out[i].size()}")
                # print(f"Debug target type is: {type(target[i])}") #, size is: {target[i].size()}")
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        num_samples=25,
        results_folder="/mnt/7T/Data/zheng/RDDM_results/",
        amp=False,
        fp16=False,
        split_batches=True,
        ...
        generation=False,
        num_unet=2,
        artifact_property_trian={'random': True, 'fixed_exposure_time': 100},
        artifact_property_val={'random': False, 'fixed_exposure_time': 100}
    ):
        super().__init__()
        ...
        if self.condition:
            if len(folder) == 2:
                self.condition_type = 6
                # test_input
                ds = dataset([folder[1]], self.image_size,
                             augment_flip=False, convert_image_to=convert_image_to, condition=11, equalizeHist=equalizeHist, crop_patch=crop_patch, sample=True, generation=generation, \
                                 artifact_property=artifact_property_val)
                trian_folder = [folder[0]]
                ...
                ds = dataset(trian_folder, self.image_size, augment_flip=augment_flip,
                             convert_image_to=convert_image_to, condition=11, equalizeHist=equalizeHist, crop_patch=crop_patch, generation=generation, \
                                 artifact_property=artifact_property_trian)
                ...
                self.sample_dataset = ds
                # print(f"Debug num_samples is {num_samples}")
                self.sample_loader = cycle(self.accelerator.prepare(DataLoader(self.sample_dataset, batch_size=num_samples, shuffle=True,
                                                                               pin_memory=True, num_workers=4)))  # cpu_count()
                ...
                ds = dataset(folder[3:6], self.image_size,
                             augment_flip=False, convert_image_to=convert_image_to, condition=2, equalizeHist=equalizeHist, crop_patch=crop_patch, sample=False, generation=generation)
        ...
        if self.num_unet == 1:
            self.model, self.opt0 = self.accelerator.prepare(
                self.model, self.opt0) # TODO: original use self.opt, which should be an error
        elif self.num_unet == 2:
        ...
        with tqdm(initial=self.step, total=self.train_num_steps, disable=not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    if self.condition:
                        data = next(self.dl)
                        # print(f"Debug in residual_denoising_diffusion_pytorch.py data is: {type(data)}, {data[0].size()}, {data[1].size()}")
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        if self.step != 0 and self.step % (self.save_and_sample_every*10) == 0:
                            ...
                            # try:
                            #     gen_img = f'{results_folder}/test_timestep_{self.model.sampling_timesteps}_' + \
                            #         str(milestone)+"_pt"
                            # except:
                            #     gen_img = f'{results_folder}/test_timestep_{self.model.module.sampling_timesteps}_' + \
                            #         str(milestone)+"_pt" 
                            gen_img = f'{results_folder}/test_timestep2358_' + \
                                str(milestone)+"_pt"   
                            self.set_results_folder(gen_img)
                            self.test(last=True, FID=False, sample=True)
                            os.system(
    ...
    def sample(self, milestone, last=True, FID=False):
        ...
        with torch.no_grad():
            ...
            if self.condition_type == 0:
                ...
                show_x_input_sample = x_input_sample
                x_input_sample = x_input_sample[1:]
            elif self.condition_type == 6:
                x_input_sample = next(self.sample_loader)
                # x_input_sample = self.sample_loader[1]
                x_input_sample = [item.to(self.device)
                                  for item in x_input_sample]
                show_x_input_sample = x_input_sample
                x_input_sample = x_input_sample[1:]
            for test_sampling_timesteps in [2,3,5]:
                start_time = clock.time()
                all_images_list = show_x_input_sample + \
                    list(self.ema.ema_model.sample(
                        x_input_sample, batch_size=batches, last=last, test_sampling_timesteps=test_sampling_timesteps))
                end_time = clock.time()
                time_recorder = (end_time - start_time) * 1000
                # print(f"Debug x_input_sample: {x_input_sample[0].size()}")
                # print(f"Debug all_images_list: {len(all_images_list)}")
                # print(f"Debug show_x_input_sample: {len(show_x_input_sample)}")
                ...
                if FID:
                    for i in range(batches):
                        file_name = f'sample-{milestone}_step{test_sampling_timesteps}.png'
                        utils.save_image(
                    ...
                    # file_name = f'sample-{milestone}_step{test_sampling_timesteps}_usedtime{int(time_recorder)}ms.png'
                    file_name = f'sample-{milestone}_step{test_sampling_timesteps}.png'
    ...
    def test(self, sample=False, last=True, FID=False):
        ...
        if self.condition:
            ...
            i = 0
            for items in loader:
                for test_sampling_timesteps in [2,3,5,8]:
                    if self.condition:
                        file_name = f"step{test_sampling_timesteps}_" + self.sample_dataset.load_name(
                            i, sub_dir=self.sub_dir)
                    else:
                        file_name = f'{i}_step{test_sampling_timesteps}.png'
                    
                    if "B_ept" not in file_name:
                        continue
                    with torch.no_grad():
                        batches = self.num_samples
                        ...
                        if sample:
                            all_images_list = show_x_input_sample + \
                                list(self.ema.ema_model.sample(
                                    x_input_sample, batch_size=batches, test_sampling_timesteps=test_sampling_timesteps))
                            end_time = clock.time()
                        else:
                    ...
                    time_recorder = (end_time - start_time) * 1000
                    all_images = torch.cat(all_images_list, dim=0)
                    # print(f"Debug no sample all_images: {all_images.size()}")
                    if last:
                        nrow = int(math.sqrt(self.num_samples))
                    else:
                        nrow = all_images.shape[0]

                    # file_name = f"usedtime{int(time_recorder)}ms" + file_name
                    utils.save_image(all_images, str(
                        self.results_folder / file_name), nrow=nrow)
    ...
    def set_results_folder(self, path):
        ...
        # print(f"Debug result folder is {self.results_folder}")
